...
`src/train.py` defines `augment_training_data()` (line ~422) which would
expand S beats 6x and V beats 2x with synthetic augmentation. **It is
currently dead code — never called.** The baseline train distribution
(N=45839, S=943, V=3788) matches the raw DS1 beat counts exactly, which
confirms it did not run. Do not wire it up: doing so violates constraint 2.
